...
namespace Marketplace.Api.Controllers
{
    ...
    public class MediaController : ControllerBase
    {
        ...
        [HttpGet("{entityId}")]
        public async Task<ActionResult<IEnumerable<MediaResponse>>> GetAllEntityMedia(Guid entityId)
        {
            if (entityId == Guid.Empty)
            {
                return BadRequest("Invalid request.");
            }
            var mediaFiles = await _mediaService.GetAllEntityMediaUrls(entityId);
        ...
        public async Task<ActionResult> UploadMedia([FromForm] UploadMediaRequest request)
        {
            ...
            await _mediaService.AddMedia(request);
        ...
        public async Task<ActionResult> DeleteMedia(Guid mediaId)
        {
            ...
            await _mediaService.SoftDeleteMedia(mediaId);
            return NoContent();
        }
        [HttpDelete("hard/{mediaId}")]
        public async Task<ActionResult> HardDeleteMedia(Guid mediaId)
        {
            if (mediaId == Guid.Empty)
            {
                return BadRequest("Invalid media ID.");
            }
            await _mediaService.HardDeleteMedia(mediaId);
            return NoContent();
        }
    }
}
